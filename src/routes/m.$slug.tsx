...
const publicMenuQuery = (slug: string) =>
  queryOptions({
...
export const Route = createFileRoute("/m/$slug")({
  loader: ({ params, context }) =>
    context.queryClient.ensureQueryData(publicMenuQuery(params.slug)),
  head: ({ params }) => ({
    meta: [
      { title: `Menu — ${params.slug}` },
      { name: "description", content: `Browse the digital menu for ${params.slug}.` },
      { property: "og:title", content: `Menu — ${params.slug}` },
...
function PublicMenuPage() {
  const { slug } = Route.useParams();
  const { data } = useSuspenseQuery(publicMenuQuery(slug));
