...
import { createFileRoute, notFound, Link, useSearch, useLocation } from "@tanstack/react-router";
import {
  getRestaurantData,
...
export const Route = createFileRoute("/$restaurantUsername")({
  ...
  loader: async ({ params }) => {
    const slug = params.restaurantUsername;

    // 1. Try to load active restaurant
    const restaurant = await fetchPublicMenu(slug);

    if (restaurant) {
      return { restaurant, restaurantUsername: slug, suspended: false, suspendedName: "" };
    }

    // 2. Restaurant not found via active query — check if it exists but is suspended
    try {
      const statusRow = await getRestaurantStatusBySlug({ data: slug });
      if (statusRow && statusRow.status === "suspended") {
        // Load full data ignoring status for the blurred background
        const suspendedData =
          (await fetchPublicMenu(slug).catch(() => null)) ||
          ({
            name: statusRow.name,
            username: slug,
            logoImage: statusRow.logo_url,
          } as unknown as import("@/lib/restaurants-data").Restaurant);
        return {
          restaurant: suspendedData,
          restaurantUsername: slug,
          suspended: true,
          suspendedName: statusRow.name,
        };
      }
    } catch {
      /* ignore */
  ...
  head: ({ loaderData, params }) => {
    const username = params.restaurantUsername || "";
    ...
    const baseTitle =
      restaurant?.name ||
      nameMap[username.toLowerCase()] ||
      username.replace(/[^a-zA-Z0-9]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
    const title = `${baseTitle} — Digital Menu`;
    const description =
      restaurant?.descriptionText ||
      restaurant?.introText ||
      `Browse the digital menu for ${baseTitle}.`;

    const faviconUrl =
      restaurant?.favicon || restaurant?.logoImage || restaurant?.logo || restaurant?.image || "";
    const ogImageUrl =
      restaurant?.socialPreview ||
      restaurant?.image ||
    ...
    if (ogImageUrl) {
      ...
      meta.push({ name: "twitter:image", content: ogImageUrl });
    ...
    if (faviconUrl) {
      links.push({ rel: "icon", href: faviconUrl });
      links.push({ rel: "shortcut icon", href: faviconUrl });
      links.push({ rel: "apple-touch-icon", href: faviconUrl });
    }
  ...
  notFoundComponent: () => {
    return (
      <div className="mx-auto max-w-2xl p-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">No active restaurant matches this username.</p>
        <Link to="/" className="mt-4 inline-block underline">
          Go home
        </Link>
      </div>
...
function PublicRestaurantPage() {
  const { restaurant, restaurantUsername, suspended, suspendedName } = Route.useLoaderData();
  const searchParams = Route.useSearch() as Record<string, string>;
  const location = useLocation();

  // Extract table token from URL path (e.g. /burgercraft/e/gb03hs or /e/gb03hs) or search params (?t=gb03hs or ?table=03)
  const pathParts = (location?.pathname || "").split("/").filter(Boolean);
  const eIdx = pathParts.indexOf("e");
  const pathToken = eIdx !== -1 && pathParts[eIdx + 1] ? pathParts[eIdx + 1] : "";

  const token = searchParams.t || searchParams.token || pathToken || "";
  const decoded = token ? decodeTableToken(token) : null;
  const tableNumber =
    decoded?.tableNo ||
    searchParams.table ||
    ...
    searchParams.t_no ||
    "";
  const branchId = decoded?.branchSlug || searchParams.branch || "";

  if (!restaurant) return null;

  return (
    <div className="relative">
      {/* Blurred menu behind overlay */}
      ...
      {/* Non-closeable suspended overlay */}
  ...
  useEffect(() => {
    async function loadAdminMenuData() {
      ...
      try {
        ...
        if (dynamicFavicon && typeof document !== "undefined") {
          const existingIcons = document.querySelectorAll<HTMLLinkElement>(
            "link[rel*='icon'], link[rel*='apple-touch-icon']",
          );
          if (existingIcons.length > 0) {
            existingIcons.forEach((el) => {
              el.removeAttribute("type");
              el.href = String(dynamicFavicon);
            });
          } else {
            const linkIcon = document.createElement("link");
            linkIcon.rel = "icon";
            linkIcon.href = String(dynamicFavicon);
            document.head.appendChild(linkIcon);
          }
        }
